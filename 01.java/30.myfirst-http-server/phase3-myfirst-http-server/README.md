...
**Java Socket을 이용한 HTTP 서버 직접 구현**

---

## 프로젝트 개요

이 프로젝트는 **Java Socket을 이용하여 HTTP 서버를 직접 구현한 교육용 프로젝트**입니다. 

Spring Boot와 같은 프레임워크 없이 **순수 Java만으로 HTTP 요청을 처리**하는 과정을 학습할 수 있습니다.
...
```
Client (curl/브라우저) → ServerSocket(8080) → Thread Pool
                             ↓
                   HttpServerLite.handleClient()
                             ↓
    ┌────────────────────────┼────────────────────────┐
    │                        │                        │
HttpRequest.parse()   Router.match()      RouteHandler.handle()
    │                        │                        │
바이트→HTTP객체        URL패턴매칭           비즈니스로직(CRUD)
    │                        │                        │
    └────────────────────────┴───────────→ HttpResponse.write()
```

**처리 흐름**: 연결 수락 → 파싱 → 라우팅 → 비즈니스 로직 → 응답 전송
...
```
┌──────────────────┐         ┌──────────────────┐         ┌──────────────────┐         ┌──────────────────┐         ┌──────────────────┐
│  바이트 스트림   │         │  ① Request Line │         │  ② Headers      │         │  ③ Body         │         │  HttpRequest     │
│                  │         │     파싱         │         │     파싱         │         │     읽기         │         │     객체         │
│ GET /users/123   │         │                  │         │                  │         │                  │         │                  │
│ Host: localhost  │ ──────> │ BufferedReader로 │ ──────> │ 빈 줄까지        │ ──────> │ Content-Length   │ ──────> │ method: GET      │
│ Content-Type:    │         │ 첫 줄 읽기       │         │ 반복 읽기        │         │ 확인 후          │         │ path: /users/123 │
│  application/json│         │                  │         │                  │         │ 바디 읽기        │         │ headers: Map     │
│ Content-Length:45│         │ → 메서드/경로/   │         │ → Map에          │         │                  │         │ body: JSON       │
│                  │         │   버전 추출      │         │   헤더 저장      │         │                  │         │                  │
│ {"name":"홍길동"}│         │                  │         │                  │         │                  │         │                  │
└──────────────────┘         └──────────────────┘         └──────────────────┘         └──────────────────┘         └──────────────────┘
```
...
(특정 사용자 조회) curl http://localhost:8080/users/1
